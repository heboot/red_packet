...
public class AlterTextActivity extends BaseActivity<ActivityAlterTextBinding> {
    ...
    public void initUI() {
        ...
        if (alterTextType == AlterTextType.NICK_NAME || alterTextType == AlterTextType.GROUP_MY_NAME || alterTextType == AlterTextType.FRIEND_NAME) {
    ...
    public void initData() {
        if (alterTextType == AlterTextType.GROUP_MY_NAME) {
            ...
            userId = getIntent().getStringExtra(MKey.ID);
        }
    }
...
//                tipDialog.show();
    ...
    private void alterFriendNickName() {
        ...
        loadingDialog.show();
        params.put("real_name", binding.etText.getText().toString());
        params.put("friend_id", userId);
        HttpClient.Builder.getServer().fRealName(UserService.getInstance().getToken(), params).observeOn(AndroidSchedulers.mainThread()).subscribeOn(Schedulers.io()).subscribe(new HttpObserver<Object>() {
            @Override
            public void onSuccess(BaseBean<Object> baseBean) {
                loadingDialog.dismiss();
                tipDialog = DialogUtils.getSuclDialog(AlterTextActivity.this, baseBean.getMsg(), true);
